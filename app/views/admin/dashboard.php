...
<div class="admin-panel-v35">
  ...
  <tr><th>Mã</th><th>Khách hàng</th><th>SĐT</th><th>Tổng tiền</th><th>Trạng thái</th><th>Ngày tạo</th><th>Chi tiết</th></tr>
  <?php foreach($orders as $o): ?>
  <tr>
  <td>#<?= $o['id'] ?></td><td><?= htmlspecialchars($o['customer_name']) ?></td><td><?= htmlspecialchars($o['phone']) ?></td><td><b><?= number_format($o['total'],0,',','.') ?>đ</b></td><td><span class="status-pill <?= gbDashStatusClass($o['status'] ?? 'Chờ xác nhận') ?>"><?= htmlspecialchars($o['status']) ?></span></td><td><?= $o['created_at'] ?></td><td><a class="order-detail-btn" href="<?= BASE_URL ?>admin/order-detail?id=<?= $o['id'] ?>">Xem</a></td>
  </tr>
  <?php endforeach; ?>
  <?php if(empty($orders)): ?><tr><td colspan="7" class="empty-admin">Chưa có đơn hàng mới.</td></tr><?php endif; ?>
  </table>
</div>
