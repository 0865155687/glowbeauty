...
(function(){
    ...
    function setCount(count){
        ...
        qsAll('.wishlist-count, .wish-count-badge').forEach(function(el){ el.textContent = count; }); if(window.GlowBeautySetWishlistCount) window.GlowBeautySetWishlistCount(count);
    }
...
(function(){
    ...
    function setWishlistCount(count){
        if(count === undefined || count === null) return;
        all('.wishlist-count, .wish-count-badge').forEach(function(el){ el.textContent = count; }); if(window.GlowBeautySetWishlistCount) window.GlowBeautySetWishlistCount(count);
    }
...
/* GlowBeauty mobile count badges final patch */
(function(){
  function ensureBadge(link, className){
    if(!link) return null;
    var badge = link.querySelector('.' + className.split(' ')[0]);
    if(!badge){
  ...
  function findCartLink(){
    return Array.prototype.slice.call(document.querySelectorAll('a')).find(function(a){
      var h=(a.getAttribute('href')||'').toLowerCase();
      return a.classList.contains('cart-btn') || h.endsWith('/cart') || h.includes('/cart');
    });
  }
  function findWishLink(){
    return document.querySelector('.wish-top-btn') || Array.prototype.slice.call(document.querySelectorAll('a')).find(function(a){
      var h=(a.getAttribute('href')||'').toLowerCase();
      return h.includes('wishlist');
    });
  }
  document.addEventListener('DOMContentLoaded', function(){
    ensureBadge(findCartLink(), 'cart-count');
    ensureBadge(findWishLink(), 'wish-count-badge wishlist-count');
  });
  window.GlowBeautySetCartCount = function(n){
    n = parseInt(n,10); if(isNaN(n)) return;
    ensureBadge(findCartLink(), 'cart-count');
    Array.prototype.slice.call(document.querySelectorAll('.cart-count')).forEach(function(el){ el.textContent = String(n); });
  };
  window.GlowBeautySetWishlistCount = function(n){
    n = parseInt(n,10); if(isNaN(n)) return;
    ensureBadge(findWishLink(), 'wish-count-badge wishlist-count');
    Array.prototype.slice.call(document.querySelectorAll('.wishlist-count,.wish-count-badge')).forEach(function(el){ el.textContent = String(n); });
  };
})();
